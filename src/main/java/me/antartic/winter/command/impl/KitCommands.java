...
import xyz.refinedev.spigot.api.knockback.IKnockbackAPI;
...
public class KitCommands implements PotPvPCommand {
    ...
    @Command(name = "setkb", usage = "<kitType> <knockback>", desc = "Set a KitType's knockback")
    @Require("potpvp.kit.admin")
    public void setKnockback(@Sender Player sender, KitType kitType, String knockback) {
        kitType.setKnockbackProfile(knockback);
        kitType.saveAsync();
        sender.sendMessage(ChatColor.GREEN + "You've updated this kit-type's knockback");
    }
